...
(def bot-token (or (System/getenv "TELEGRAM_BOT_TOKEN") "1948800776:AAE5V9P_mNNPxL0IcKuRDbKKY8u1lxq2Mf4"))
(def bot-webhook-url (or (System/getenv "TELEGRAM_BOT_WEBHOOK") "https://super-dice-roll-discord-clj.herokuapp.com/telegram/webhook"))
...
(defn set-webhook-url []
  (-> (build-commands-url "setWebhook")
      (curl/post {:headers {"Accept" "application/json"
                            "Content-Type" "application/json"}
                  :body (json/encode {:url bot-webhook-url})
                  :throw true})
      (dissoc :headers)
      println))
